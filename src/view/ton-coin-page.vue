<template>
  ...
  >
    ...
    <button
      ...
      class="flex bg-[#1D2633] h-[48px] px-[20px] mt-[30px] items-center rounded-[30px]"
    >
      <img width="30px" src="../assets/toncoin.svg" alt="" />
      <p class="ml-[5px] text-center">Add +5 ton</p>
    </button>
  </div>
  ...
  >
    <div class="text-center pb-[20px] pt-[30px] relative">
      ...
      <h1 class="font-bold text-[25px]">{{ selectedItem.ton }}</h1>
      <p class="mb-[25px]">
        {{ selectedItem.date }}
      </p>
...
export default {
  ...
  computed: {
    ...
    filteredData() {
      return this.items.filter((item) => item.transfer === "ton");
    },
  },
  methods: {
    selectItem(item) {
